...
test("公開スマホの絞り込みはURLと点数を保ち、メニューから戻れる", async ({ page }, info) => {
  ...
  await expect(filters).toContainText(/\d+ 点/);
  await filters.getByRole("button").click();
  const dialog = page.getByRole("dialog", { name: "写真を絞り込む" });
  await expect(dialog).toBeVisible();
  await dialog.getByRole("group", { name: "撮影方式" }).getByRole("button", { name: "Film", exact: true }).click();
  ...
  await expect(dialog.getByRole("button", { name: `${filmCount} 点の写真を見る`, exact: true })).toBeVisible();
  await page.keyboard.press("Escape");
  await expect(filters.getByRole("button")).toBeFocused();
  await page.locator(".gallery-mobile-filters__selection").getByRole("button", { name: "解除" }).click();
