...
>
> ## 必要性の報告 — 本番適用はオーナーの直接依頼がある場合だけ
>
> この節は移行が必要な理由を示すもので、実行指示ではない。`db:push` / `db:migrate` は
> **オーナーの直接依頼がない限り実行しない**。直接依頼がない場合、Claude / Codex を
> 含むエージェントは必要性・対象DB・想定される影響を報告するだけにする。
>
> オーナーが本番適用を直接依頼した場合だけ、次を実行候補として扱う。
>
> ```bash
> bun run db:push
> ```
>
> この計画で確認済みなのは、想定SQLが列を7つ追加する形であることだけである。
> 実際の本番DBの状態と表示されるSQLを確認するまで、安全とは断定しない。コードをpushする
> 前後を問わず、直接依頼とSQL確認の両方が必要である。
>
