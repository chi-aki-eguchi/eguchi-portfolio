...
const photos = [
  ...
  { id: 2, sortOrder: 0, shotAt: null, createdAt: "2026-01-01T00:00:00Z" },
...
describe("sortPhotosBySetting", () => {
  ...
  test("date sorts keep undated photos last and fall back to manual ties", () => {
  ...
  test("upload_desc sorts by createdAt and falls back safely", () => {
    expect(sortPhotosBySetting(photos, "upload_desc").map((p) => p.id)).toEqual([1, 3, 2]);
  });
