...
### 追記 — A層の削減と wiki 鮮度チェック（2026-08-20、同日 第2回）

§6-A と §8-3(c) を実行した。

**A層 288行 → 239行（−49）。**内訳は `AGENTS.md` 148→110、`CLAUDE.md` 64→55、
`file-hygiene.md` 18（変更なし）、Current State 58→56。
§6-A の見積り235行との差4行は、`CLAUDE.md` が55行（53の見積りに対し）、
`AGENTS.md` が110行（109に対し）、Current State が56行（55に対し）であるため。
**数字を合わせるために内容を削るのはやめた。**

（commit `ab2406e` のメッセージは241行と書いているが、同じcommitで Current State を
58→56行に書き直したため、確定値は239行である。commit の書き換えはしない規則なので
ここで訂正する。）

削除前に、削る内容がB側に実在するかを19項目すべて照合した。**4項目は実在せず、
先にB側へ移してから削った。**

| 移した内容 | 移設先 |
|---|---|
| 最小Handoff の7項目 + 30〜60行の上限 | `docs/agents/handoff-workflow.md`「Current State に残す項目」 |
| オーナーへの報告の書き方（3項目） | 同「オーナーへの報告」 |
| 参照先のリンク集 | `docs/README.md`（新規） |
| 「過去 memory を採用しない」規則 | `CLAUDE.md` 前文（Codex とは無関係の規則なので Codex 節と一緒に消さなかった） |

**削らなかったもの:** §0 不変条件24行、「絶対に越えない境界」9行（push禁止・本番DB・
秘密情報）、「オーナーの直接指定がそのタスクだけ優先される」の1行
（`codex-workflow.md` に無かったため `AGENTS.md` に残した）。

**wiki 鮮度チェック**を `scripts/ai/check-wiki-freshness.mjs` として追加し、
`bun run check` の最後に置いた。45日超のページを一覧で警告する。
**終了コードは常に0で、check を失敗させない**（`--strict` を明示した時だけ1）。
テスト10件が、境界（45日ちょうどは警告しない）、`last_verified` 欠落時に
**今日ではなく最終commitの日付**を提案すること、常に0で返ることを見張る。

初回実行で **13ページ中10ページが45日超**。日付は動かしていない。動かすと、
このチェックが見つけるはずの問題を隠すことになる。

---

### 追記 — 夜間ラン前提の記述の洗い出し（2026-08-20、第3回）

対象は `AGENTS.md` / `CLAUDE.md` / `docs/agents/` / Current State の4箇所。
「無人で動くこと」「朝に人が読むこと」を前提にした記述を、語彙と構造の両面から探した。
検索語: 夜間 / 夜 / 無人 / 自走 / ループ / 朝 / 就寝 / 起きたら / 翌朝 / 一晩 /
night / overnight / unattended / セッションレポート / cron / schedule / tick / 各周 /
closing / 枠が切れ / 放置 / オーナー不在 / 判断して進 / 人が読 ほか。

**結果: 対象4箇所には、ほぼ残っていない。**該当は1件だけだった。
2026-08-20 の第1回で `refine-and-loop-spec.md` / `night-run.md` /
`autonomy-rules.md` / `wake-up-checklist.md` / `claude-code-night-run.md` /
`improvement-roadmap.md` を archive へ移したことで、実体はそのとき落ちている。

#### 対象4箇所での該当（1件）

| # | 場所 | 記述 | 判定 |
|---|---|---|---|
| N-1 | `docs/agents/codex-workflow.md:165` | 「**closing以下では**新しい工程を始めず…」 | **要確認。削除しない。** |

`closing` は旧クレジット判定の段階名で、**現在の `credit-status.md` には
定義が無い**（2026-08-03 に取得状態 / 週枠 / 作業継続性の3軸へ作り直され、
「旧形式の`status`は判断に使わない」と明記されている）。
つまり参照先が消えた宙ぶらりんの語である。

ただし**文そのものは夜間ラン専用ではない**。「残量が少ないときは新しい工程を
始めず、Git状態と次の一手を Current State へ残す」は、有人でも5時間枠が
尽きかけたときにそのまま要る。**削除案ではなく語の差し替え案**として挙げる。

> 案: 「closing以下では」→「週枠が `low`、または作業継続性が `at_risk` /
> `unknown` のときは」

依頼が「報告のみ・勝手に消さない」だったため、**この変更は当てていない。**

#### 対象範囲外で見つかったもの（参考。触っていない）

| # | 対象 | 内容 | 判定 |
|---|---|---|---|
| N-2 | `docs/archive/night-20260707.md`（60行。2026-08-20 に `docs/reports/` から移設） | **「夜間自律改善レポート 2026-07-07」。**冒頭が「作業ブランチ `improve/night-20260707`、コミット8件。**push はしていません。**内容を確認のうえ、マージ・push は秋さんの判断で」——**朝にオーナーが読む前提で書かれた成果物そのもの。** | **明らかに夜間ラン専用。archive 行き候補。** |
| N-3 | ローカルブランチ `improve/night-20260707` | N-2 が指す作業ブランチ。origin には無く、ローカルにだけ残っている | **要確認**（8コミットの中身が main に入ったか未確認のため、消す判断はオーナー） |

**N-2 を今回消さなかった理由:** 対象範囲（4箇所）の外であることに加え、この
レポートが `ws` / `js-yaml` の脆弱性と R2 サムネイルのリークの**当初記録**であり
（§7-1・§7-3 がここを出典にしている）、削除すると経緯の出所が消える。
`docs/archive/` へ移すのが妥当だが、範囲外なのでオーナー判断とする。
なおこのレポートの内容は既に古い——§7-3 のリークは解消済みである。

#### 「夜間ラン専用ではない」と判断して残したもの

紛らわしいが、有人運用でも要るため対象にしなかった。理由を添える。

- `AGENTS.md`「止まって報告する条件」— 無人時の自動判断ではなく、有人でも
  効く停止の安全弁。
- `credit-status.md`「再開可能な区切りを作ると安全です」— リセット駆動の
  自走ループ由来だが、**現在は5時間枠に対して効いている**（この作業中にも
  実際に表示された）。
- `handoff-workflow.md`「停止・復帰」— 無人の引き継ぎではなく
  Claude ↔ Codex の受け渡し。
- `credit-status.md` の hook 3点（SessionStart / UserPromptSubmit / Stop）—
  有人セッションで毎回動いている。
...
- `docs/checklists.md` ほか `docs/` 直下 — 今回の対象範囲外。
  夜間レポートは 2026-08-20 に `docs/archive/night-20260707.md` へ移設した。
  §7-1 と §7-3 の当初記録だが、§7-3 のリークは**もう解消済み**なので、
  あの報告書の内容自体は現状と食い違っている（履歴として残す）。
