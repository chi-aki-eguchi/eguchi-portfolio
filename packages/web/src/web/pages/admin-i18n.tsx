  ...
  library: {
    ...
    sort: {
      ...
      options: {
        manual: "手動順",
        manualShort: "手動",
        uploadedNewest: "追加が新しい順",
        uploadedNewestShort: "アップロード日 新しい順",
        uploadedOldest: "追加が古い順",
    ...
    filters: {
      ...
      missingTitle: "題名なし",
      emptyLead: "まだ埋まっていない",
      emptyTitle: (n: string | number) => `題名がない ${n}`,
      emptyCategory: (n: string | number) => `分類がない ${n}`,
      emptySeries: (n: string | number) => `シリーズに入っていない ${n}`,
    ...
    preview: {
      ...
      shortcutInspector: "写真を編集",
      shortcutPreview: "クイックプレビュー",
      shortcutDelete: "ゴミ箱へ移動",
  ...
  settingsBasic: {
    ...
    pageLayout: {
      ...
      bookUnusedNotes: {
        ...
        "page-layout": "写真中心の構成では「シリーズの札の形」は使いません（Series のページは決まった組み方です）。「トップの言葉」は、表紙のすぐ下か、写真の一覧の後に出ます。",
        "font-size": "写真中心の構成では、表紙の名前の大きさは名前の欄の幅から自動で決まります（ここの「名前の大きさ」は使いません）。ほかの大きさと全体の倍率は効きます。",
        "site-copy": "写真中心の構成では「TOP」「Gallery」「Works」「すべて見る」「ギャラリー見出し」の言葉は使いません（それらの場所がありません）。About・Contact・SNS・絞り込みの「すべて」・お問い合わせの言葉は効きます。",
    ...
    fade: {
      ...
      appearanceLabel: "現れ方",
      appearanceHint:
        "コラージュのような枠・傾きつきレイアウトでは、動きのあるもの（浮き上がり・ズーム・寄る・下りる・起き上がる）より「フェード」「なし」が自然に見えます",
      names: {
        fade: "フェード",
        none: "なし",
        rise: "浮き上がり",
      ...
      descriptions: {
        ...
        rise: "少し下から上がってくる",
        scale: "小さくから拡大して現れる",
        "drift-left": "少し右から流れてくる",
        "drift-right": "少し左から流れてくる",
        drop: "少し上から下りてくる",
        tilt: "わずかに傾いた状態から起きる",
      },
    },
...
const ADMIN_PHASE_2B_EN = {
  library: {
    ...
    filters: {
      ...
      emptyLead: "Still empty",
      emptyTitle: (n: string | number) => `No title ${n}`,
      emptyCategory: (n: string | number) => `No category ${n}`,
      emptySeries: (n: string | number) => `In no series ${n}`,
  ...
  settingsBasic: {
    ...
    pageLayout: {
      ...
      bookUnusedNotes: {
        ...
        "page-layout": "The photographs-first design does not use the series card style. The home statement appears right under the cover or after the photographs.",
        "font-size": "In the photographs-first design the cover name is sized to fit its column, so the name size here is not used. The other sizes and the global scale apply.",
        "site-copy": "The photographs-first design does not use the TOP, Gallery, Works, View all and gallery heading labels. About, Contact, social, the filter’s All and the contact form labels apply.",
      } as Record<string, string>,
    ...
    fade: {
      ...
      appearanceHint:
        "For framed, tilted layouts like Collage, “Fade” or “None” tend to look more natural than the moving options (Rise, Zoom, Drift, Descend, Straighten).",
      names: {
        fade: "Fade",
        none: "None",
        rise: "Rise",
        scale: "Zoom",
        "drift-left": "Drift from right",
        "drift-right": "Drift from left",
        drop: "Descend",
        tilt: "Straighten",
      },
      descriptions: {
        ...
        "drift-left": "Drifts in slightly from the right",
        "drift-right": "Drifts in slightly from the left",
        drop: "Descends slightly from above",
        tilt: "Straightens up from a slight tilt",
      },
    },
    galleryLayout: {
