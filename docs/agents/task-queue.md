...
- [~] 公開サイト側の点検（2026-07-26 に着手）— /gallery のスマホ横スクロールを修正済み。
      **未着手の発見が2件ある**:
      - **本文のコントラストが AA 未達**。opacity 0.55 の文字を #f7f7f7 に重ねると
        合成 rgb(125) で **3.82:1**（必要4.5:1）。T-11 のコミットは「0.55 で 5.26:1 達成」と
        書いているが、5.26:1 になるのは opacity 0.65 相当。**T-11 の検証が誤っていた**。
        プロフィールの経歴文・ナビの名前・セクション見出しなどが該当。
        AA に必要な opacity は **0.60 以上**（0.62 で約4.77:1）。
        ただし `--foreground` / `--background` は site_settings 由来で
        オーナーが変更できるため、固定値で直すか計算で出すかは設計判断が要る
      - **画像が表示サイズに対して過大**。例: 792px表示に1920px、230px表示に640px、
        208px表示に800px。design-spec §7 の srcset 方針と突き合わせる
