...
- [x] 動き（2026-07-26 点検）— **公開サイトは既に design-spec §6 のとおりだった**。
      ...
      許容範囲。**課題として立てない**
- [ ] **ボタンの枠の有無が揃っていない**（2026-07-26 実測で発見）。同じツールバー内で
      `border-width: 1px` のものと `0px` のものが混在している
      （「選択モードを開始」「絞り込み」「Table」は1px / 「サイトで確認」「Trash」
      「キーボードショートカット」は0px）。到達点(4)「同じ操作は同じ見た目」に反する
