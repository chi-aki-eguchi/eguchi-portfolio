...
### 2026-08-09 追加依頼 — ページ最上部の帯を選べるようにする

オーナー依頼「上の帯の白を無くす（文字だけまたは薄いフェード）も選択できるように」。
設定キー `headerBackground` = `solid`（既定） / `fade` / `none`。

**地を透かすだけでは帯は消えない。** 裏が本文の余白なら、同じ色の帯が残るだけ。
実測（本番）でも全画面HEROは header の下ではなく **56px 下から**始まっていた。
そこで透かすときは、全画面HEROを header の下まで伸ばして写真そのものを地にする。
`.header-see-through .site-main:has(.hero-fullscreen)` で、**全画面HEROを含む
本文だけ**に効かせる。ギャラリーや About まで潜らせると見出しが帯に隠れる。

**踏んだ罠:** `main` の上余白がインライン style だったため、CSS の上書きが常に
負けていた。`.site-main` クラスへ移して解決。
...
|---|---|---|---|
| 本文の上余白 | 56px | **0px** | **0px** |
| HERO の上端 | 56px | **0px** | **0px** |
| ヘッダーの地 | `rgb(18,18,18)` | 同色のグラデーション | **`rgba(0,0,0,0)`** |
| 文字の暈し | なし | あり | あり |

全画面HERO以外（gallery / about / contact / series / HERO=normal のTOP）は
`none` のままでも余白 56px を保ち、見出しは帯の下に出ることを確認。
スマホの開閉メニューは不透明な面を持つので影響なし。

**正直な限界:** 「文字だけ」は写真の明暗に左右される。明るい配色のまま暗い写真を
使うと、暗い文字が沈んで読みにくい（実測で確認）。暈しは広く薄い2層から狭く濃い
3層へ変えて改善したが、それでも限界がある。管理画面の注意書きにそう明記し、
迷うときは「薄いフェード」を勧める形にした。
...
## 過去 Current State — 2026-08-31 JST（動きの直しの前）
...
| `ceb4b08` | `/work` を OGP の既知経路へ／Profile の締めの帯が押しのけられるのを止める（0.0634→0.0016） |
| `511b015` | **`/work` の HTTP 404 は3つ目の一覧が原因だった。**経路一覧の突き合わせテストを新設 |
...
- **`.env` は Railway が正**（オーナー確認）。`packages/web/.env` はどこからも
  読まれていない死んだ複製だったので、オーナーの指示で**削除した**。
  キーの顔ぶれはルートと同一で、失われた情報は無い（値は記録しない）。
  Vite も DB 系スクリプトもルートの `.env` を読む。
- **`/profile` のズレは直した**（0.0634 → 0.0016）。画面に出ている帯が読んで
  いる最中に押しのけられていたため。`min-height` では直らない（帯はその節の
  **中**にある）。上の中身が出そろってから帯を出す形にした。
- **触らないと決めたもの**: ページ移動の真っ白な324ms（意図した幕。好みの領域）、
  絞り込まない `/gallery` で締めの帯がほぼ出ないこと（出すと読んでいる最中に
  飛ぶ。実測 0.751）。

### 公開ページを増やすときに必ず触る場所（2026-08-31 に痛い目を見た）

`/work` を足したとき、`app.tsx` には入れたのに他を忘れて**本番が HTTP 404**を
返していた（画面は出るので気づけない。**開発サーバは何でも 200 を返す**）。
一覧は5か所ある:

1. `web/app.tsx` の `<Route>`
2. `api/public-routes.ts` の `SPA_STATIC_PATHS` … **HTTP の status**
3. `api/ogp.ts` の `KNOWN_ROUTES` … **題名と robots**
4. `api/route-preload.ts` の `ROUTE_MODULES` … 先読み
5. `server.ts` の sitemap

2・3・4 には突き合わせテストがあるので、忘れれば `bun run check` が止める。
5 は自動では見ていない。
...
## 過去 Current State（2026-09-01 に差し替え）
...
## 過去 Current State（2026-09-02 に差し替え）
