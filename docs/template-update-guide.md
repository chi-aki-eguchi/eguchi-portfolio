...
- トップに写真が表示される
- 写真を置いている一覧に既存写真がある。見る場所はサイトによって違います。
  - ナビに Gallery があるサイト: `/gallery`
  - ナビに Gallery がないサイト（写真をすべて Series／Work に置いている）: ナビの
    Series または Work から作品を1つ開く。この形では `/gallery` を直接開いても
    写真は並ばず、Series・Work への案内が出ます。**それが正常で、故障ではありません。**
- `/admin/login` の画面が開く

更新後に同じ場所で見比べるため、どのページで確認したかを控えておきます。
...
4. 公開サイトを再読み込みし、次を確認する。
   - トップと、手順1で確認した一覧（`/gallery`、または Series／Work の作品）に
     更新前の写真が残っている
