...
| `fab5804` | **admin**: 名乗っている仕事をしていなかった段差を外す（上の訂正）|

**通底する発見: 直すべきは duration ではなかった。**公開サイトは前のめりの
カーブが時間を見えない所へ捨てていた。ライトボックスは DOM から外していて
transition がそもそも走れなかった。admin の段差は**当たり先が中身ではなく
骨組み**だった（`.admin-screen` の孫＝仕切り線とプレビュー枠。Series では
1件しか当たらず何も起きていなかった）。**どれも「長さを足す」では直らない。**

**動きを足すより、名乗っている仕事をしていない動きを外す方が効いた。**
効いているつもりを消すには、セレクタが何件に当たっているかを実物で数える。

    90%到達（実測）        前     後
      写真1枚             333ms  615ms
      節・ページ・帯       258ms  483ms
      ぼけが晴れる         264ms  750ms
      admin 画面切替       150ms  236ms（6px → 10px。実測）
