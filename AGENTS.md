...
- 管理役は要件整理・分解・設計判断・最終確認を行い、仕様と完成条件が明確な実装・反復修正・テスト追加は `gpt-5.3-codex-spark` を優先する。既存の調査担当は維持する。
- このMacでは `~/.local/bin/codex-spark` をCodex・Claude Codeで共用する。手順とタスク票は `~/.local/share/codex-spark/README.md`。目的・対象ファイル・変更禁止範囲・完成条件・テストと必要な文脈だけを渡す。
- Sparkは専用worktreeで単独編集し、管理役は終了後に関連差分をまとめてレビューする。同じ問題で修正2回失敗なら管理役へ戻す。呼べない場合は報告し、通常モデルやAPI課金へ自動代替しない。Sparkにpush・本番反映・DNS/ドメイン変更を渡さない。残量不明は未確認と報告する。
