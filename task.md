...
| `e1212d7` | 販売ページに **Product + Offer**（値段は servicePageConfig から読む）と FAQPage。題を検索の言葉から始める |
| `69e466f` | B-25 を backlog へ |

### 検証

- `bun run check` = **1158 pass / 0 fail**（exit 0）
- `bun run smoke` = **356 passed / 0 failed**（146 skipped・13.4分）
...
**判断待ち:** 販売ページの題と説明の文面。オーナーが実物を見て決める。
いまの案は `写真家のポートフォリオサイト制作 | Aki Eguchi Portfolio Kit`。
