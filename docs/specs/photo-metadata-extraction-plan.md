...
> ## 移行は適用済み（2026-08-20 確認）
>
> **本番 Turso の `photos` は36列で、この計画の7列はすべて存在する。**
> 2026-08-20 に読み取り専用で実測した（`node scripts/ai/check-prod-photo-columns.mjs`、
> `PRAGMA table_info(photos)` の1本だけ）。移行前の29列から7列増えており、
> `docs/archive/wake-up-checklist.md` の予行演習（29→36）と一致する。
>
> **したがって、以前ここにあった「7列が無いとアプリが動かない」という警告は
> もう成り立たない。**その警告は 2026-07-28（`d0b6bdf`）に書かれ、移行が当たった
> あとも消されずに残っていた。2026-08-18 の `bun run smoke` が6回とも
> 330〜331 passed / 0 failed で通っている事実とも矛盾していた。
>
> ### DB移行の扱い（規則。この節は残す）
>
> `db:push` / `db:migrate` は**オーナーの直接依頼がない限り実行しない**。
> 直接依頼がない場合、Claude / Codex を含むエージェントは必要性・対象DB・
> 想定される影響を報告するだけにする。実行時は流すSQLを必ず目視し、
> `ADD COLUMN` 以外（`DROP` / `CREATE TABLE` / データ移動）が出たら中止する。
> 照合用SQLは `packages/web/drizzle/0005_mysterious_madame_masque.sql`。

**この文書は起票時（2026-07-28）は実装前の設計だったが、単位3〜7は実装済みで、
7列は本番にも適用されている。以降の本文は当時の設計内容そのままである。**
...
5. 本番適用は**オーナーの直接依頼がある場合だけ**行う。直接依頼がない場合は実行せず、必要性だけを報告する
...
オーナーから本番適用の直接依頼がある場合でも、**`bun run db:migrate` は使わないほうがよい。** 本番DBの `__drizzle_migrations`
に 0000〜0004 が記録されていなければ、`0000`（CREATE TABLE）から流そうとして失敗する。
記録されているかは本番を見ないと分からない。

オーナーから本番適用の直接依頼がある場合の**推奨は `bun run db:push`**。本番の実際のschemaと `schema.ts` を比べ、
差分だけを流す。今回は列の追加7本だけなので、生成された `0005` と同じ
`ALTER TABLE ... ADD COLUMN` になるはず。

> **オーナーから直接依頼を受けた場合だけ**: `db:push` は実行前に「これから流すSQL」を表示する。
...
> `db:push` と `db:migrate` は**本番DBに直接当たる**。オーナーの直接依頼がない限り、
> Claude / Codex を含むエージェントは実行せず、生成と必要性の報告までで止める。
