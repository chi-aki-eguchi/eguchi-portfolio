...
export const DEFAULT_SERVICE_CONFIG: ServicePageConfig = {
  ...
  purchaseFlow: {
    ...
    steps: [
      ...
      {
        title: "自分で立てる場合",
        body: "立ち上げ用リンクと手順書を見ながら公開します。操作方法の相談も含みます（当面は期間の制限なし）。",
      },
      {
        title: "公開おまかせの場合",
        body: "写真・プロフィール・連絡先などの素材が揃ってから3日以内に、サイトURL、管理画面URL、パスワードを渡します。操作方法の相談も含みます（当面は期間の制限なし）。",
      },
    ],
    footnote:
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: "購入すると、どこまで利用できますか？",
        a: "1回の購入につき1サイトで利用できます。2サイト目以降は追加購入が必要です。テンプレートとしての再販売・再配布はできません。テンプレート本体の更新は現時点では無償で提供しますが、今後変更する場合があります。操作方法の相談は、どちらのプランも当面は期間・回数の制限なく受け付けます（今後、期間制に変更する可能性があります）。デザイン変更・個別カスタム・作業の代行は、内容に応じて別途お見積もりします。",
      },
      {
        q: "やめたいときはどうなりますか？",
