...
## 2026-09-19 追記 — 13件をすべて直した（全体 smoke が初めて失敗0）

上の13件（backlog S-3）を1件ずつ実物で確かめた。**5件は spec の古さではなく製品の不具合**で、
テストが通っていなかったせいで隠れていた。全体 `bun run smoke` は 575成功・180スキップ・**失敗0**
（直前は 565成功・177スキップ・失敗13。合計755は同じで、10件が成功へ、3件が project 指定によるスキップへ移った）。

### spec が製品に追いついていなかったもの（5件）

| spec | 直したこと |
|---|---|
| admin-contact-settings-validation:271 | 共通ナビが読む `/api/photos/availability` を人工データで返す（2026-09-16 `d82d461` で増えた通信）。「想定外の通信」の意味は弱めていない |
| admin-library-modes:163 | 絞り込み中の「並べ替え」は押せなくするのではなく**押したら解除して入る**（2026-09-13 の意図した動作）。押す前は並べ替えに入っていないこと、押したあと条件が本当に消えていることを足した |
| admin-library-contact-sheet:63 ×3 | 題のとおり PC の導線を測る検査なので project を desktop に限定。スマホの並べ替えは admin-mobile-library.spec.ts が見ている |
| admin-workspace-layout:103 | 広い画面のナビ幅は 2026-09-13 の再設計で 240px → 216px。期待値だけが古かった |
| admin-form-layout:196 | 目次の幅を 207/163 の固定値で縛っていた。**ナビの内側と一致し、読める幅（150px以上）がある**を測る形へ。次に寸法が変わっても腐らない |

### spec がスマホの導線を知らなかったもの（3件）

admin-library-modes:16・:292、admin-library-selection:20。スマホ幅には3つ並ぶ切替が出ず、
「選択」は操作帯の専用ボタン（`data-library-mobile-select`）、並べ替えからの脱出は「並べ替えを終了」、
選択の終了は「選択終了」になる。`helpers.ts` の `gotoAdminTab` と spec の `enterMode` がこれを見るようにした。

**残る差（仕様の判断が要る・直していない）**: 選択したまま並べ替えへ入り「並べ替えを終了」すると、
スマホでは選択が消える。PCは並べ替え→選択へ直接戻れて復元する（`applyLibraryMode` の `keepSelection` は
arrange→select だけ）。スマホにはその道が無い。テストにはこの事実をそのまま書いてある。

### 製品の不具合だったもの（5件・修正済み）

1. **矢印キーが常に写真一覧に奪われる**（admin-workspace-layout:37 の原因）。畳んだナビのグループは
   ArrowRight で自分のメニューを開くのに、`admin.tsx` の全体キー処理が先に写真一覧へ飛ばし、焦点ごと
   持っていっていた。キーボードだけで使うと、どこを触っても写真へ引き戻される。写真一覧の中か、
   どこにも焦点が無いときだけ矢印・Home/End を受け持つようにした。
2. **畳んだナビのメニューを開いても最初の項目に焦点が当たらない**。`requestAnimationFrame` 1回で待って
   いたが、React の反映より先に走ることがあった（jsdom は間に合うので単体テストでは気づけない）。
   開いたあとの effect で当てる。
3. **写真をダブルクリックすると2つ開く**（admin-i18n:7・admin-selected-button:55・
   admin-workspace-layout:103 の後半の原因）。1打目で詳細が開き、**2打目が開いたばかりの画面の同じ座標**
   に当たって、PCでは「拡大」が一緒に開き、スマホでは節の切替が勝手に動いていた。詳細の枠で、開いた
   直後（500ms以内）に届く連打の2打目だけ捨てる。
4. **スマホの一括メニューが画面の外へ出る**（admin-library-selection:20 の後半）。選択中に「シリーズ」を
   開くと、いちばん長い名前に合わせてメニューが幅504pxになり、375pxの画面で左端が x=-212.6 だった。
   押したボタンではなく操作帯に合わせて開き（包みを `position: static` に）、長い名前は折り返す。
   実測は x=12・幅351（375px の画面）。
5. 目次の幅（上の表の admin-form-layout）は、寸法の期待値そのものが壊れやすい作りだった。

### 記録

証拠は `scratch/smoke-evidence/` の各実行フォルダ。スマホの一括メニューの修正後の様子は
`scratch/site-improve-20260919/mobile-batch-menu-after.png`。
写真一覧の仮想表示の検査（admin-debug-sweep:195）は、人工データで一覧に出る写真25枚では動かずスキップになる
（これは失敗ではない）。
