...
- 通常の製品改善は、上記検証後にcommit・push・本番確認まで進める。ローカルのみ・調査のみの依頼はその範囲に従う。文書整理だけで公開作業を追加する必要はない。
- `main` へのpushはRailwayの本番反映につながる。`git fetch origin` と送るcommitを確認し、送る内容を把握して通常のpushを使う。公開済みの作業は普段のmainへ統合し、完了したworktree・ブランチを残し続けない。
