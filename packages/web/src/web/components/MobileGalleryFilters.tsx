  ...
  return (
    <div className="gallery-mobile-filters">
      <div className="gallery-mobile-filters__bar">
        <span aria-live="polite">{loading ? "読み込み中" : failed ? "読み込めませんでした" : `${count}枚`}</span>
        {(categories.length > 1 || hasMedium) && (
          <button type="button" onClick={() => setOpen(true)} aria-expanded={open}
            ...
            写真を探す <ChevronDown size={13} aria-hidden="true" />{selected.length > 0 && <span className="gallery-mobile-filters__badge">{selected.length}</span>}
          </button>
        )}
      </div>
      {selected.length > 0 && <div className="gallery-mobile-filters__selection">
        <span>{selected.join(" / ")}</span>
        <button type="button" onClick={() => onChange({ c: "all", medium: "all" })}>すべての写真</button>
      </div>}
      ...
      {open && <dialog id="gallery-filter-dialog" ref={dialogRef} className="gallery-filter-dialog"
        ...
        <header><h2 id="gallery-filter-title">写真を探す</h2>
          <button type="button" onClick={() => setOpen(false)} aria-label="写真を探すメニューを閉じる"><X size={20} /></button>
        </header>
        <div className="gallery-filter-dialog__body">
          {categories.length > 1 && <fieldset><legend>テーマ</legend><div>
        ...
        <footer>
          <button type="button" disabled={selected.length === 0} onClick={() => onChange({ c: "all", medium: "all" })}>すべての写真</button>
          <button type="button" className="gallery-filter-dialog__apply" onClick={() => setOpen(false)}>{count}枚の写真を見る</button>
        </footer>
      </dialog>}
    </div>
  );
