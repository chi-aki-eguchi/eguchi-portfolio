...
2. LP 全文の英語版（コース A/B/C、FAQ、購入の流れ、24時間以内案内・3日以内納品の約束、
   買い切り条件、上記「正直な制約」）
3. JP/EN 切り替えリンクを LP 両言語の右上に置く（互いにリンクするだけ。状態保存しない）
...
## Phase 3 — 海外購入者が実際に現れてから

- 管理画面 UI の英語化(重い。distribution-ideas #12。買い手が付くまでやらない)
- 手順書全訳、USD 決済リンク、英語サポートテンプレート
...
LP 英語文面のドラフト（docs 上でレビュー可能に）→ ルート+辞書実装 → OGP/hreflang/sitemap →
スタートページ+メール文面 → render テスト → check/smoke → Handoff
