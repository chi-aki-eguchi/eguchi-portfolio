...
- **Status:** デバッグ3巡目まで完了（設定 → 静的解析 → 操作 → **スマホadmin**）。
  合計19件を修正。**settings 155キーは全て「効く」ことを確認済み**。
  commit 済み・push 未実施
- **Current owner:** Claude Code（停止中）
- **Branch:** `main` / **HEAD:** `SELF` / **origin/main:** `03d22c3` より 12 commits ahead
...
3. 残りは `docs/agents/backlog.md`。今回 B-15〜B-21 を追加した。
   うち **B-19（絞り込みがURLに入らない）と B-21（スマホでLibraryの写真名が出ない）は
   オーナー判断待ち** — どちらも設計の話。

### 検証の状態

- `bun run check` **成功**（674 pass / 0 fail）
- `bun run smoke` **成功**（304 passed / 0 failed。回帰テストを20件追加）
