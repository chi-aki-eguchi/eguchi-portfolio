...
## Q-11. Library: スマホ幅の写真詳細を「下側シート」にする [昼推奨] [Codexレビュー必須]
...
承認済み設計（`docs/specs/library-redesign-spec.md` §2-2）は
「390px: 詳細は下側シートとして全幅表示」であり、実装が設計と食い違っている。

2026-07-25 にオーナー判断で、モード分離とは切り分けて次段の別タスクとした
（モード分離＝操作の分離、詳細の出し方＝別の設計項目のため）。

やること:
- 390px で写真詳細を下側シート（全幅・画面下から出る）として表示する
- 詳細を開いたままでもモード切替に手が届くことを保証する
- 1024px の「右から重ねて表示」、1440px の挙動は変えない
...
- 実ブラウザ 390px と iPhone 実機 Safari で目視
