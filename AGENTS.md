...
- 主担当が目的・設計判断・実装・検証を一貫して扱う。サブエージェント・別AI・Sparkへの委譲はユーザーが明示した場合だけ使い、固定の司令塔・実装・レビュー工程を置かない。
- 引き継ぐ場合は目的・受け入れ条件・決定理由・変更箇所・実施済み検証・未解決を渡す。編集担当は1人とし、受け取った要約は必要な実物と照合する。
...
- smokeは実行ごとの一時SQLite・人工データ・127.0.0.1の偽ストレージで動き、本番のDB・保存先・`.env`を使わない（`scripts/smoke/isolated-server.ts`）。接続先を確認できない・ポートが使用中のときは起動しない。書き込みはspec側のモックで確認し、本番へSave/Deleteを確定しない。specからAPIを直接読むときは `scripts/smoke/fixtures.ts` の `api` を使う（`page.request` などは塞いである）。
