...
1. 残量に加えて**リセットまでの時間**と**このペースで持つか**を判断へ入れる
2. `unknown` を追加し、「情報が取れない」を深刻度の軸に乗せない
3. 5hセッションの枯渇は、週枠が持つ限り「工程を止める理由」にしない
   （数時間待てば戻るため、範囲を縮めるのではなく区切り方の問題として扱う）

未実装。実装前に反対レビュー（Phase C）を通すこと。特に
**「残量が本当に少ないのに検出だけ壊れている」場合に事故らないか**を名指しで検証する。
