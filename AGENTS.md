...
- `Content-Encoding` を付けるのは `api/http-compression.ts` **だけ**。他の場所で手で付けない。
  ...
  これが 2026-06-13 の二重圧縮事故。Railway プロキシは HTML と `/api` は gzip で
  圧縮するが、**`/assets/*` は圧縮しない**（immutable でedgeにキャッシュされる分。
  2026-08-23 実測）。だから origin 側でも圧縮する。測り方は
  `docs/agents/measuring.md`。HTMLは `Cache-Control: no-store`。
